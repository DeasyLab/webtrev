module OutplanparamsHelper
  def pagination_links_remote(paginator)
    page_options = {:window_size => 1}
    pagination_links_each(paginator, page_options) do |n|
      options = {
        :url => {:controller=>"inplanparams", :action => 'list', :params => @params.merge({:page => n})},
        :update => 'table',
  ...
  def sort_link_helper(text, param)
    ...
    options = {
      ...
      :url => {:controller=>"inplanparams", :action => 'list', :params => @params.merge({:sort => key, :page => nil})},      
      :before => "Element.show('spinnerSTR')",
      :success => "Element.hide('spinnerSTR')"
    }
    html_options = {
