module UserstructdictsHelper
  ...
  def sort_link_helper(text, param, action)
    ...
    options = {
      :url => {:controller => 'userstructdicts', :action => '#{action}', :params => @params.merge({:sort => key, :page => nil})},
      :update => 'settingDisp',
      :before => "Element.show('spinnerLM')",
      :success => "Element.hide('spinnerLM')"
    }
    html_options = {
      ...
      :href => url_for(:controller => 'userstructdicts', :action => '#{action}', :params => @params.merge({:sort => key, :page => nil}))
    }
